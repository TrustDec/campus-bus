  ...
  <div>
    <div v-if="dataStart">
      <div class="advertisement">
        ...
        <img src="../common/images/banner.png" class="advertisement-image">
      </div>
      ...
      this.$http.get(url).then(response => {
       ...
       // console.log(data.index.length);
        for (var i = 0; i < data.index.length; i++) {
          data.index[i].beizhu = data.index[i].beizhu+'';
          if (data.index[i].beizhu.length == 0) {
            data.index[i].beizhu ='null'
          }
        }
